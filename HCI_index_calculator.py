...
# CIT = f((T, A) * P)
# T: thermal, A: aesthetic, P: physical
# CIT: 1 ~ 7, 1=poor, 7=ideal
## TSN: thermal facet of climate: very hot ~ very cold: +4 ~ -4
## C_rate = 6.4 + 0.4 * TSN - 0.281 * TSN ** 2
#3S (sun, sea and sand)

"""
However, as previously noted, the index suffers from a
number of limitations. The central weakness of the TCI is that the rating and weighting scheme of the
sub-indices are subjectively based on Mieczkowski’s opinion and not empirically tested against the
preferences of tourists or any other tourism performance metrics. The results from multiple stated
preference surveys have revealed that urban tourists are more accepting of a broader range of climatic
conditions than initially conceived by Mieczkowski 

file:///Users/gimgibeom/Downloads/atmosphere-07-00080.pdf
"""
...
---> Designed to be specified for major tourism segments and destiantion type

HCI: Urban = 4(TC) + 2(A) + (3(precipitation) + wind) : 'urban tourism'

Each climatic variable is rated on a scale from 0 to 10
overall HCI idx: 0 to 100-> 0: bad, 100: ideal
...
#     t: mean temparature of the air
#     h: relative humidity
#     v: the saturation vapour pressure(hPa), 
#        and can be estimated as v = 6.11*10^(???)
#     """
#     RSI = (10.7 + 0.74*(t - 35)) / (44. - 0.0075*h*v)
#     return RSI
